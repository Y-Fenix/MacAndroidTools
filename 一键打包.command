#!/bin/zsh
set -u

SCRIPT_DIR="$(cd "$(dirname "$0")" && pwd)"
VENV_DIR="$SCRIPT_DIR/.venv"
PYTHON_BIN="$(command -v python3 || true)"

pause_on_error() {
  local exit_code="$1"
  if [ "$exit_code" -ne 0 ]; then
    echo
    echo "打包失败，按回车键关闭窗口..."
    read
  fi
  exit "$exit_code"
}

if [ -z "$PYTHON_BIN" ]; then
  echo "未找到 python3，请先安装 Python 3。"
  pause_on_error 1
fi

if [ ! -d "$VENV_DIR" ]; then
  echo "正在创建虚拟环境..."
  "$PYTHON_BIN" -m venv "$VENV_DIR" || pause_on_error 1
fi

PYTHON_IN_VENV="$VENV_DIR/bin/python"
PIP_IN_VENV="$VENV_DIR/bin/pip"

if [ ! -x "$PYTHON_IN_VENV" ]; then
  echo "虚拟环境损坏，正在重建..."
  rm -rf "$VENV_DIR"
  "$PYTHON_BIN" -m venv "$VENV_DIR" || pause_on_error 1
fi

ICON_SOURCE=""
if [ -f "$SCRIPT_DIR/Android.ico" ]; then
  ICON_SOURCE="$SCRIPT_DIR/Android.ico"
elif [ -f "$SCRIPT_DIR/android.ico" ]; then
  ICON_SOURCE="$SCRIPT_DIR/android.ico"
else
  echo "未找到 Android.ico 或 android.ico。"
  pause_on_error 1
fi

echo "正在安装打包依赖..."
"$PIP_IN_VENV" install --upgrade pip >/dev/null || pause_on_error 1
"$PIP_IN_VENV" install -r "$SCRIPT_DIR/requirements-build.txt" || pause_on_error 1

echo "正在生成 macOS 图标..."
"$PYTHON_IN_VENV" "$SCRIPT_DIR/make_icns.py" --input "$ICON_SOURCE" --output "$SCRIPT_DIR/Android.icns" || pause_on_error 1

echo "正在执行 PyInstaller 打包..."
cd "$SCRIPT_DIR" || pause_on_error 1
"$PYTHON_IN_VENV" -m PyInstaller \
  --noconfirm \
  --clean \
  --windowed \
  --name AndroidTools \
  --icon "$SCRIPT_DIR/Android.icns" \
  --add-data "$ICON_SOURCE:." \
  "$SCRIPT_DIR/androidtools.py" || pause_on_error 1

echo
echo "打包完成：$SCRIPT_DIR/dist/AndroidTools.app"
open "$SCRIPT_DIR/dist" >/dev/null 2>&1
